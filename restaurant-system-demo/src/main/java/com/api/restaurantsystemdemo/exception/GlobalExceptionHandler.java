...
public class GlobalExceptionHandler {
	...
	protected ResponseEntity<MessageDTO> handleException(Exception ex) {
		...
		return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(new MessageDTO("Exception não tratada: " + ex.toString(), "com.api.restaurantsystemdemo.data.integrity.violation.exception"));
	}
	
//	@ExceptionHandler(DataIntegrityViolationException.class)
//	protected ResponseEntity<MessageDTO> handleDataIntegrityViolationException(DataIntegrityViolationException ex) {
//	    logger.error("Exceção de Integridade de Dados: ", ex);
//	    return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new MessageDTO("Exceção de Integridade de Dados: " + ex.getMessage(), "com.api.restaurantsystemdemo.data.integrity.violation.exception"));
//	}
}